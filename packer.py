...
class BinFactory(object):
    ...
    def __init__(self, width, height, weight, count, bid, pack_algo, *args, **kwargs):
        self._width = width
        self._height = height
        self._weight = weight
        self._count = count
...
class PackerOnline(object):
    ...
    def add_bin(self, width, height, weight, count=1, bid=None, **kwargs):
        # accept the same parameters as PackingAlgorithm objects
        kwargs['rot'] = self._rotation
        bin_factory = BinFactory(width, height, weight, count, bid, self._pack_algo, **kwargs)
...
class Packer(PackerOnline):
    ...
    def add_bin(self, width, height, weight, count=1, bid=None, **kwargs):
        self._avail_bins.append((width, height, weight, count, bid, kwargs))
    ...
    def pack(self):
        ...
        for b in self._avail_bins:
            width, height, weight, count, bid, extra_kwargs = b
            super(Packer, self).add_bin(width, height, weight, count, bid=bid, **extra_kwargs)
